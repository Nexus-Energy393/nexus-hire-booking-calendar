...
async function createAsset(a) {
  const cols = ["fleet_number","asset_name","category","generator_size_kva","make","model",
    ...
    "last_service_hours","service_due_warning_hours","location","status","notes"];
...
async function listAllocations(filter) {
  ...
  const sql = "SELECT * FROM allocations" +
    (where.length ? " WHERE " + where.join(" AND ") : "") +
    " ORDER BY hire_start NULLS LAST, created_at";
  return db.query(sql, params);
}
...
async function createAllocation(a) {
  const cols = ["pipedrive_deal_id","booking_title","asset_id","stock_item_id","quantity_required",
    "quantity_allocated","allocation_status","hire_start","hire_end","cross_hire_qty","override_note","notes"];
  const params = cols.map(function (c) { return a[c] != null ? a[c] : null; });
  const placeholders = cols.map(function (_, i) { return "$" + (i + 1); }).join(", ");

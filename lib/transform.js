...
const F = {
  ...
  electrical: process.env.PD_FIELD_ELECTRICAL_CONNECTION_REQUIRED || "c75a45614d94ed7ca9d2f38aeee4f54330efbd5f",
  ...
  additionalEquipment: process.env.PD_FIELD_ADDITIONAL_EQUIPMENT || "4e4fffbf0a89903ff3a9015ab5535eecb2a72b6e",
  electricalInspection: process.env.PD_FIELD_ELECTRICAL_INSPECTION_REQUIRED || "a46d8ba6104de6e27944611628b69a4e5d9c5af6",
  refuelling: process.env.PD_FIELD_REFUELLING_REQUIRED || "fdbfc04a19a8c7dbec42fd7e8965446623aea48c",
  safetyItems: process.env.PD_FIELD_SAFETY_ITEMS || "bef805ba07a0e668a5b320a2e4bee438de5a668a",
  tradingOpen: process.env.PD_FIELD_TRADING_HOURS_OPEN || "ef174003f2a4070b44f546420bf9937235d602c6",
  tradingClose: process.env.PD_FIELD_TRADING_HOURS_CLOSE || "ef174003f2a4070b44f546420bf9937235d602c6_until",
  open24h: process.env.PD_FIELD_OPEN_24H,
  deliveryFreight: process.env.PD_FIELD_DELIVERY_FREIGHT || "0d79a509d158823c31002c74ef2507d8d4b970ff"
};
...
/* Yes/No enum -> boolean (resolves option id to its label first). null when unset. */
function yesField(deal, optionLabels, key) {
  if (!key) return null;
  var v = readField(deal, optionLabels, key);
 ...
 */
/* Resolve a deal field by its Pipedrive NAME (lowercased) using extras.fieldsByName. */
function keyByName(extras, name) {
  var m = (extras && extras.fieldsByName) || {};
  return m[String(name).trim().toLowerCase()] || null;
}
function nameYes(deal, extras, name) {
  var k = keyByName(extras, name);
  if (!k) return false;
  return yesField(deal, extras.optionLabels || {}, k) === true;
}
function nameVal(deal, extras, name) {
  var k = keyByName(extras, name);
  if (!k) return "";
  return readField(deal, extras.optionLabels || {}, k) || "";
}
/* Build the medical-equipment label list from the Yes/No equipment fields. */
function readMedicalEquipment(deal, extras) {
  var items = [];
  if (nameYes(deal, extras, "X-Ray Equipment")) items.push("X-Ray");
  if (nameYes(deal, extras, "Imaging Equipment")) items.push("Imaging/CT");
  return items;
}
...
function dealToBooking(deal, extras) {
  ...
  const prospective = !!extras.prospective;
  const status = prospective ? "prospective" : resolveStatus({
    startISO: startISO,
  ...
  return {
    ...
    dealStage: extras.stageName || "",
    deliveryRequired: truthy(cf(deal, F.delivery)),
    electricalConnectionRequired: yesField(deal, optionLabels, F.electrical),
    ...
    electricalInspectionRequired: yesField(deal, optionLabels, F.electricalInspection),
    refuellingRequired: yesField(deal, optionLabels, F.refuelling),
    ...
    solarPresent: nameYes(deal, extras, "Solar Installed"),
      standbyGenerator: nameYes(deal, extras, "Standby Generator"),
      medicalFacility: nameYes(deal, extras, "Medical Facility"),
      medicalEquipment: readMedicalEquipment(deal, extras),
      googleEventId: null
  };
}

...
/* Parse period string → { start: Date, end: Date, label: string }
...
function computeUtilisation(staffList, allocations, unavailability, periodStart, periodEnd) {
  ...
  return staffList.map(function (member) {
    ...
    for (const u of myUnavail) {
      ...
      // Clamp each unavailability block to at most 8 h/business-day
      // Simple approach: just count overlap hours, capped at stdHours per block max
      unavailHours += Math.min(oh, stdHours);
    }
    unavailHours = Math.min(unavailHours, stdHours); // can't subtract more than available
    ...
    for (const a of myAllocs) {
      // Clamp allocation to period window via overlap
      const aStart = new Date(a.allocation_start);
      const aEnd   = new Date(a.allocation_end);
      const allocDuration = (aEnd - aStart) / 3_600_000 || Number(a.duration_hours) || 0;
      const periodDuration = (periodEnd - periodStart) / 3_600_000 + 24; // inclusive end
      const allocationPeriod = (aEnd - aStart) / 3_600_000 || 1;
      // Proportional share inside the query window
      const oh = overlapHours(aStart, new Date(aEnd.getTime() + 86_400_000), periodStart, new Date(periodEnd.getTime() + 86_400_000));
      // Use recorded duration_hours proportionally
    ...
    // ── detect overlapping allocations (conflicts) ──

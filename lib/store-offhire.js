...
/* TODAY, IN MELBOURNE.
 *
 * Postgres CURRENT_DATE resolves in the SESSION timezone, and nothing sets one
 * on the Neon pool, so it is UTC. Melbourne is UTC+10/+11, which means that
 * between local midnight and 10am (11am in daylight saving) the database still
 * thinks it is YESTERDAY. The off-hire queue is the first thing the yard reads
 * in the morning, and for that whole window it said "nothing due" while units
 * sat overdue in the field, then healed itself mid-morning - which reads as
 * flaky rather than broken. Everything date-shaped here is Melbourne local. */
const MELB_TODAY = "((now() AT TIME ZONE 'Australia/Melbourne')::date)";
...
async function offHireEquipment(rec) {
  ...
  /* THE 007 COLUMN, not the 003 one.
   *
   * Migration 007 moved fuel out of a parsed note into fuel_out_pct /
   * fuel_return_pct / ongoing_refuel, and store-fleet.recordEngineHours was
   * updated to write them. This path was not: it kept writing the old
   * fuel_level_return_pct, which nothing reads. So the fuel level captured on
   * the Off Hire return form went into a column no screen, no PDF and no CRM
   * mirror ever looks at, isFuelRecorded stayed false for those hires, and the
   * dispatch gate 007 exists to close kept failing open. Both columns are
   * written: the new one because it is the one that is read, the old one so a
   * row written today still reads correctly to anything still on the old name.
   *
   * fuel_used_litres is COALESCEd rather than overwritten - re-submitting an
   * off-hire with no fuel figure used to wipe a total that was already there. */
  const returnPct = rec.fuel_return_pct != null ? rec.fuel_return_pct
                  : (rec.fuel_level_return_pct != null ? rec.fuel_level_return_pct : null);

  let record;
  if (open) {
    record = await db.queryOne(
      "UPDATE engine_hour_records SET hours_in = $1, runtime_hours = $2, " +
      "fuel_used_litres = COALESCE($3, fuel_used_litres), " +
      "fuel_return_pct = COALESCE($4, fuel_return_pct), " +
      "fuel_level_return_pct = COALESCE($4, fuel_level_return_pct), " +
      "recorded_by = COALESCE($5, recorded_by), " +
      "notes = COALESCE($6, notes) WHERE engine_hour_record_id = $7 RETURNING *",
      [rec.hours_in, runtime, fuelLitres, returnPct,
    ...
    record = await db.queryOne(
      ...
      "fuel_used_litres,fuel_return_pct,fuel_level_return_pct,recorded_by,notes) " +
      "VALUES ($1,$2,$3,$4,$5,$6,$7,$7,$8,$9) RETURNING *",
      [rec.asset_id, rec.pipedrive_deal_id, hoursOut, rec.hours_in, runtime, fuelLitres,
       returnPct, rec.recorded_by || null, rec.notes || null]);
  }
...
async function offHiresDueRaw() {
  return db.query(
    ...
    // The real service figures. These were not selected, so listOffHiresDue
    // substituted last_service_hours = 0 and let the interval default to 300 -
    // which made nextDue 300 for every machine and flagged the entire fleet
    // overdue. A flag that is always on carries no information, and hid the
    // units that genuinely were due.
    "  ast.last_service_hours, ast.service_interval_hours, ast.service_due_warning_hours, " +
    "  (" + MELB_TODAY + " - a.hire_end) AS days_overdue, " +
    ...
    // <= not <. A hire ending TODAY is due back today: the queue is
    // "equipment to check in", and off-hire.js even has a "Due today" label
    // that was unreachable because this was strict.
    "  AND a.hire_end IS NOT NULL AND a.hire_end <= " + MELB_TODAY + " " +
    "ORDER BY a.hire_end ASC", []);
}async function listOffHiresDue() {
  const rows = await offHiresDueRaw();
  return rows.map(function (r) {
    const svc = R.serviceStatus({
      current_engine_hours: r.current_engine_hours,
      last_service_hours: r.last_service_hours,
      service_interval_hours: r.service_interval_hours,
      service_due_warning_hours: r.service_due_warning_hours
    });

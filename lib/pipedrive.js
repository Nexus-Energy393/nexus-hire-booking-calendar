...
let _stagesCache = new Map();   // pipelineId -> { at, data }
...
/* Pipeline stages (cached). Used to resolve the "Negotiations Started" stage id
   by name so prospective (open) deals can be filtered without hard-coding an id. */
async function getStages(pipelineId) {
  const key = String(pipelineId || "all");
  const hit = _stagesCache.get(key);
  if (_fresh(hit, FIELDS_TTL_MS)) return hit.data;
  const params = pipelineId ? { pipeline_id: pipelineId } : undefined;
  const json = await pd("/stages", params);
  const data = json.data || [];
  _stagesCache.set(key, { at: Date.now(), data: data });
  return data;
}

/* All OPEN deals in the hire pipeline. Paginated. Used to surface prospective
   (in-negotiation) jobs on the calendar before they are won. */
async function getOpenHireDeals() {
  const pipelineId = process.env.PIPEDRIVE_HIRE_PIPELINE_ID;
  const out = [];
  let start = 0;
  let more = true;
  while (more) {
    const json = await pd("/deals", { status: "open", limit: 100, start: start });
    const data = json.data || [];
    data.forEach((d) => {
      if (!pipelineId || String(d.pipeline_id) === String(pipelineId)) out.push(d);
    });
...
module.exports = {
  ...
  getWonHireDeals, getOpenHireDeals, getStages, isHireDeal, BASE, DOMAIN
};

...
The **Nexy CRM** is the single **source of truth**. When a deal is **won in the HIRE pipeline**, the CRM exposes it on its read-only "Hire Operations" feed (`/api/hire/calendar`); this app reads that feed, and shows each booking on a clean calendar designed for the office screen and for staff desktops. The app is strictly **read-only** against the CRM - it never creates or edits deals. Pipedrive has been retired as the data source.

> **Status: LIVE.** The board reads real won hire/outage deals from the Nexy CRM Hire Operations feed on every load (with a short cache) and renders them across all views. If the feed is unreachable it falls back to bundled sample data so the screen is never blank.

- **Live app:** https://nexus-hire-booking-calendar.vercel.app/
- **Source of truth:** Won deals in the Nexy CRM HIRE pipeline, via `https://nexus-crm-gilt.vercel.app/api/hire/calendar` (read-only).
...
- Reads **won deals in the HIRE pipeline** live from the Nexy CRM Hire Operations feed.
...
- Click any booking for full detail and a deep link back to the deal in the Nexy CRM.
...
- `api/bookings.js` - **GET /api/bookings**. On each request it fetches the Nexy CRM Hire Operations feed (`HIRE_FEED_URL`) and returns `{ ok, count, bookings }`. The CRM does the deal->booking shaping; this function just fetches, caches (default 60s; bypass with `?refresh=1`) and passes through, then reconciles fleet allocation dates.

The mapping from won CRM deal to booking lives on the CRM side (`src/lib/hire-calendar.ts` in the nexus-crm repo), so the board carries no Pipedrive client or transform any more.

The front-end calls `/api/bookings` on load. If it returns bookings it runs in **live mode** ("Live data - synced from the Nexy CRM hire pipeline" in the footer); if the call fails or returns nothing it falls back to the bundled `sample-data.js` so the screen is never empty.

## 3. Field mapping (in the CRM feed)

The CRM feed maps each won HIRE-pipeline deal into the booking shape. Deal identity: bookings keep a numeric `pipedriveDealId` (the deal's preserved Pipedrive id, so existing fleet allocations/notes still match) plus `crmDealId` and `crmUrl` for the deep link into Nexy.

| Booking field | CRM source |
|---|---|
| Hire start / end date | `PlannedOutageFields.outageStartDate` / `outageEndDate` (honours the TBA flags) |
| Job type | `Deal.type` (PLANNED_POWER_OUTAGE / ALDI -> planned-outage, else general) |
| Generator size / model | `Deal.generatorModel` (falls back to `ContactUsWebpageFields.generatorSizeRequired`) |
| Equipment / fleet ID | `Deal.serialFleetNumber` |
| Site / suburb / state | `ContactUsWebpageFields.siteAddress` / `siteSuburb` / `siteState` |
| Duration | derived from start/end, else `Deal.hirePeriods` x `hirePeriodUnit` |
| Customer / contact / owner | Organisation / Person / Owner on the deal |
| Outage window, map link | `PlannedOutageFields.timeOff`-`timeOn`, `mapLink` |
| Solar / medical / standby flags | `SiteSurveyFields` |
...
- `HIRE_FEED_TOKEN` - optional. Only needed if the CRM has `HIRE_FEED_TOKEN` set; then use the SAME value here. Leave blank if the feed is open.

Optional / has sensible defaults:

- `BOOKINGS_CACHE_SECONDS` - in-memory cache window for /api/bookings (default 60).
...
**Live mode:** the deployed app calls `/api/bookings`, which reads the Nexy CRM Hire Operations feed (`HIRE_FEED_URL`). Use `/api/bookings?refresh=1` to bypass the cache, or the **Refresh now** button in the UI. The **Sync Status** tab shows mode, source of truth, total bookings and last refresh.

## 7. Google Calendar mirror (optional, not yet wired)

A one-way visibility mirror to a Google "Nexus Generator Hire Bookings" calendar is on the roadmap. The Nexy CRM remains the source of truth; Google Calendar would be a read-only mirror that cannot hold the structured hire data.
...
- Live data depends on the CRM Hire Operations feed being reachable; otherwise the board runs in sample mode.
- Bookings whose CRM deal has **no hire start date** appear under **Missing Info** / **needs-review** until the dates are entered in Nexy. This is a data-entry prompt, not an app fault.
- `Delivery required` and `Electrical connection required` are read from the deal's Contact/Site fields in the CRM; blank values render as No.
- Read-only against the CRM by design - never creates or edits deals.
...
*Built for Nexus Energy operations. Nexy CRM = source of truth (read-only). This app = operational booking board.*
...
Buttons: **Print jobsheet**, **Open Nexy deal** (deep link into the CRM), **Mark ready for dispatch** (local toggle - see below).
...
| `HIRE_FEED_URL` | Nexy CRM Hire Operations feed the board reads | Calendar falls back to sample data |
